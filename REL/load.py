...
def load_data(name,maxpadlen,balanced,cpf):
    ...
    sequences = sequence.pad_sequences(sequences,maxpadlen)
    #sequences = tokenizer.texts_to_sequences(train)
    #length = max(map(len, sequences))
    #sequences = np.array([xi+[None]*(1017-len(xi)) for xi in sequences])
    ...
    arguments=np.amax(sequences) + 1    
    print(arguments-1,'unique tokens')
    maxlen=sequences.shape[1]
    print('maximum words:',maxlen)
    return (train_x,train_y,test_x,test_y,arguments,maxlen)
